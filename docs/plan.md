...
| Acting only on evidence that points at the miner (2026-09-22, 0.10.0) | Both log cursors are a place in the log (line index, line count, a hash of the line), never a timestamp; a first read stops at the newest process start, a clock jump or 5 min. A stall, or an unreachable miner whose web backend still answers, is not restarted while the log shows the pool down (`upstream_restart_hours`, 8: then one restart). The power rung reads the meter first (`unpowered_watts`, 10). An unreachable miner that cannot be seen (plug silent, meter at working power, this PC's LAN link down) is sent nothing and costs nothing from the caps; the ladder starts from zero when the path returns. | The owner's brief: "robust for things going down unpredictably and coming back up in weird orders", with no one pressing Hold in a dark house. The 2026-09-22 cable-pull test measured the outage: shares stop at once, pool lines at +2 to +2.5 min, port 4028 dead at +8 min with the web backend still up, recovery by reconnection. 0.9.1 would have soft-restarted every ~10 min to the cap. Minor bump: the watchdog's decisions, a config key each in `watchdog` and `power`, and a field in `/api/health` |
